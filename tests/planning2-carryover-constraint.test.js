...
test('central rank is TL, then SV/STV, then identity-based fair tie break',()=>{let get=(e,d)=>d==='close'?shift('09:00','19:10'):shift();assert.deepEqual(carryover.rankPlanning2CarryoverCandidates(employees,get,'close','morning').map(e=>e.id),['tl','sv','normal']);let regular=[{id:'b'},{id:'a'}],ranked=carryover.rankPlanning2CarryoverCandidates(regular,get,'close','2026-10-03').map(e=>e.id),reversed=carryover.rankPlanning2CarryoverCandidates([...regular].reverse(),get,'close','2026-10-03').map(e=>e.id);assert.deepEqual(ranked,reversed)});
...
test('08:55 rotates by prior monthly count and ignores master-data order',()=>{const people=[{id:'z'},{id:'a'}],values={};for(const day of ['2026-10-01','2026-10-02','2026-10-03'])values[day]={z:shift('09:00','19:10'),a:shift('09:00','19:10')};values['2026-10-01'].a.start='08:55';const get=(employee,date)=>values[date]?.[employee.id];for(const order of [people,[...people].reverse()])assert.equal(carryover.rankPlanning2CarryoverCandidates(order,get,'2026-10-02','2026-10-03')[0].id,'z')});

test('08:55 equal-count tie rotates rather than permanently favoring employee ID or list order',()=>{const people=[{id:'z'},{id:'a'}],values={},get=(employee,date)=>values[date]?.[employee.id];let previous=null,winners=[];for(let day=1;day<=6;day+=1){const morning=`2026-10-${String(day+1).padStart(2,'0')}`,closing=`2026-10-${String(day).padStart(2,'0')}`;values[closing]??={};values[morning]??={};for(const employee of people){values[closing][employee.id]??=shift('09:00','19:10');values[closing][employee.id].end='19:10';values[morning][employee.id]=shift('09:00','15:00')}const order=day%2?people:[...people].reverse(),winner=carryover.rankPlanning2CarryoverCandidates(order,get,closing,morning)[0].id;values[morning][winner].start='08:55';winners.push(winner);if(previous)assert.notEqual(winner,previous);previous=winner}assert.deepEqual(new Set(winners),new Set(['a','z']))});

test('08:55 initial equal-count winner changes across months and is independent of master order',()=>{const people=[{id:'z'},{id:'a'}],get=(employee,date)=>shift('09:00',date.endsWith('-01')?'19:10':'15:00'),winners=['2026-10-02','2026-11-02'].map(morning=>{const closing=morning.replace(/02$/,'01'),forward=carryover.rankPlanning2CarryoverCandidates(people,get,closing,morning)[0].id,reverse=carryover.rankPlanning2CarryoverCandidates([...people].reverse(),get,closing,morning)[0].id;assert.equal(forward,reverse);return forward});assert.notEqual(winners[0],winners[1])});
