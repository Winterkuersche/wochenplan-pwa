...
test('coverage uses every employee resolved shift and refreshes after a changed shift', () => {
  ...
  assert.deepEqual(Array.from(coverage(employees, plan, day)), [false, 'Unterbesetzung am Abend · 18:00–19:10 <2']);
...
test('persisted planning 2 edit is the entry used by coverage on the next render', () => {
  ...
  assert.deepEqual(Array.from(renderedCoverage[0]), [false, 'Unterbesetzung am Abend · 18:00–19:10 <2']);
  ...
  assert.equal(JSON.parse(stored.get('planning-2-test-plan')).schedule[dayIso].late.end, '19:00');
  assert.deepEqual(Array.from(renderedLateCells[1]), ['14:00–19:00', 'L', 'shift']);
...
test('coverage excludes resolved non-working states including external help', () => {
  ...
  assert.deepEqual(
    ...
    [false, 'Unterbesetzung am Vormittag · 9:00–19:10 <2']
  );
});
...
test('coverage identifies morning, afternoon, and evening understaffing', () => {
  const cases = [
    ...
    ['Abend', '18:00', '19:10', 'Unterbesetzung am Abend · 18:00–19:10 <2']
  ];
