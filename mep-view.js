  ...
  return rowTypes
    .map((rowType, index) => {
      ...
      const summaryColumns =
        index === 0
          ? `
            <td rowspan="4" class="mepTplSummary">${employee ? minutesToHM(getEmployeeAccountMinutesForWeek(employee, weekDays)) : ""}</td>
            <td rowspan="4" class="mepTplSummary">${employee ? minutesToHM(getEmployeeAccountMinutesForMonth(employee, state.activeMonth)) : ""}</td>
          `
          : "";
